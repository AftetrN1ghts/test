import cPickle, zlib, sys, asyncore, AreaDestructibles, BigWorld, constants, CommandMapping, ResMgr
...
import MusicController, TriggersManager
...
def _enter_offline_hangar():
    try:
        import offline_hangar
        offline_hangar.enter()
    except:
        LOG_CURRENT_EXCEPTION()
...
def start():
    ...
    if len(sys.argv) > 2:
        ...
        gui_personality.start()
        BigWorld.callback(5.0, _enter_offline_hangar)
    try:
        import Vibroeffects
...
_PYTHON_MACROS = {
    ...
    'sc': 'from account_helpers.SettingsCore import g_settingsCore; sc = g_settingsCore; sc',
...
def convertKeyEvent(event):
    ...
    return (isDown, key, mods, isRepeat)


def convertMouseEvent(event):
    return (event.dx, event.dy, event.dz, event.cursorPosition)
